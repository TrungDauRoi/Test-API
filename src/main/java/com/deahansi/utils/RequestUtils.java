...
import com.deahansi.constant.RequestConstant;
...
public class RequestUtils {
  ...
  public static String extractBrowser(HttpServletRequest request) {
    ...
    if (userAgent.contains("msie")) {
      ...
      browser =
          (userAgent.substring(userAgent.indexOf("safari")).split(StringUtils.SPACE)[0]).split(
              "/")[0] +
              "-" +
              (userAgent.substring(userAgent.indexOf(VERSION)).split(StringUtils.SPACE)[0]).split(
                  "/")[1];
    } else if (userAgent.contains("opr") || userAgent.contains(OPERA)) {
      if (userAgent.contains(OPERA)) {
        browser = (userAgent.substring(userAgent.indexOf(OPERA)).split(StringUtils.SPACE)[0]).split(
            "/")[0] +
            "-" +
            (userAgent.substring(userAgent.indexOf(VERSION)).split(StringUtils.SPACE)[0]).split(
                "/")[1];
      } else if (userAgent.contains("opr")) {
        browser = ((userAgent.substring(userAgent.indexOf("opr")).split(StringUtils.SPACE)[0])
            .replace("/", "-"))
            .replace("opr", "Opera");
      }
    } else if (userAgent.contains("chrome")) {
      browser = (userAgent.substring(userAgent.indexOf("chrome"))
          .split(StringUtils.SPACE)[0]).replace("/", "-");
